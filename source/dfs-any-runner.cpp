...
bool DfsAnagramSearch::exact_memo_lookup(
  ...
  DfsAnySolutionRunner::Memo const& memo = *worker->memo;
...
void DfsAnagramSearch::exact_memo_store(
    DfsAnySolutionRunner::Worker* worker, uint64_t exact_key, bool value) {
  DfsAnySolutionRunner::Memo& memo = *worker->memo;
  assert(memo.slots.get() != NULL);
...
bool DfsAnagramSearch::exact_class_fits(
    size_t class_index, DfsAnySolutionRunner::Worker const& worker) const {
...
bool DfsAnagramSearch::exact_candidates_lookahead(
    DfsAnySolutionRunner::Worker* worker, size_t letters_left) {
  assert(worker->memo != NULL);
  size_t const lookahead = worker->lookahead;
  assert(lookahead > 0);
...
#if 0  // Replaced below by DfsAnySolutionRunner.
bool DfsAnagramSearch::find_completable_classes(
  ...
  search_stats.any_solution = AnySolutionStats();
  ...
  search_stats.run.search_threads = 1 + pool.size();
  body(0);
  for (size_t i = 0; i < pool.size(); ++i)
    pool[i].join();
  for (size_t class_index = 0; class_index < classes.size(); ++class_index)
    (*completable)[class_index] = results[class_index] != 0;
  for (size_t i = 0; i < search_stats.run.search_threads; ++i) {
    merge_search_worker(workers[i]);
    search_stats.any_solution.classes_checked += checked[i];
    search_stats.any_solution.bound_rejects += bound_rejects[i];
    search_stats.any_solution.exact_bound_accepts += bound_accepts[i];
    search_stats.any_solution.exact_validations += exact_validations[i];
    search_stats.any_solution.memo.add(workers[i].memo);
    search_stats.any_solution.lookahead.add(workers[i].lookahead);
  }
  search_stats.run.search_seconds = std::chrono::duration<double>(
  ...
  if (progress_enabled) {
    dfs_diagnostic(
        ...
        search_stats.any_solution.classes_checked,
        search_stats.any_solution.bound_rejects,
        search_stats.any_solution.exact_bound_accepts,
        search_stats.any_solution.exact_validations);
    dfs_diagnostic(
        "phase 2 exact memo: %zu states computed, %zu hits\n",
        search_stats.any_solution.memo.states,
        search_stats.any_solution.memo.hits);
    dfs_diagnostic(
        ...
        (unsigned long long)search_stats.any_solution.lookahead.full_windows,
        (unsigned long long)search_stats.any_solution.lookahead.known_true_wins,
        (unsigned long long)search_stats.any_solution.lookahead.reprobes_decided,
        (unsigned long long)search_stats.any_solution.lookahead.recursive_expansions);
  }
  return true;
}
#endif

DfsAnagramSearch::DfsAnySolutionRunner::DfsAnySolutionRunner(
    DfsAnagramSearch& search)
    : search(search), lookahead(exact_memo_lookahead_choice()) {}

bool DfsAnagramSearch::DfsAnySolutionRunner::prepare_memo(
    DfsClassSpan classes) {
  uint64_t packed_root_key;
  if (!dfs_checked_multiply_u64(
          search.exact_root_key, UINT64_C(2), &packed_root_key) ||
      !dfs_checked_add_u64(
          packed_root_key, UINT64_C(2), &packed_root_key)) {
    dfs_diagnostic_to_stream(
        stderr,
  ...
  if (search.progress_enabled)
    dfs_diagnostic("phase 2 exact memo table: %zu slots, %llu bytes\n",
                   memo.capacity, (unsigned long long)table_bytes);
  return true;
}

bool DfsAnagramSearch::DfsAnySolutionRunner::run(
    std::vector<bool>* completable) {
  DfsClassSpan const classes = search.class_list->classes();
  if (!classes.empty()) search.require_hot_classes();
  if (!prepare_memo(classes)) return false;
  search.search_stats.any_solution = AnySolutionStats();
  completable->assign(classes.size(), false);
  std::vector<unsigned char> results(classes.size(), 0);
  size_t const worker_count = std::min(
      search.requested_search_threads, std::max(size_t(1), classes.size()));
  std::atomic<size_t> next_class(0);
  ...
  for (size_t i = 0; i < worker_count; ++i) {
    ...
    worker.exact_key = search.exact_root_key;
    worker.lookahead = lookahead;
  ...
  auto const body = [&](size_t worker_index) {
    ...
    for (;;) {
      size_t const class_index = next_class.fetch_add(1, std::memory_order_relaxed);
      if (class_index >= classes.size()) break;
      ++checked[worker_index];
      assert(search.exact_class_fits(class_index, worker));
      uint64_t parent_bag_mask;
      search.subtract_exact_class(class_index, &worker, &parent_bag_mask);
      ExactResultSource source;
      bool const result = search.exact_remainder_completable(
          &worker, search.letters.size() - classes[class_index].key_length,
          &source);
      search.restore_exact_class(class_index, &worker, parent_bag_mask);
      results[class_index] = result;
      if (source == EXACT_RESULT_BOUND_NO) ++bound_rejects[worker_index];
      else if (source == EXACT_RESULT_BOUND_YES) ++bound_accepts[worker_index];
      else if (source == EXACT_RESULT_SEARCH) ++exact_validations[worker_index];
    }
  };
  std::vector<std::thread> pool;
  pool.reserve(worker_count - 1);
  ...
  for (size_t i = 0; i < classes.size(); ++i) (*completable)[i] = results[i] != 0;
  for (size_t i = 0; i < search.search_stats.run.search_threads; ++i) {
    search.search_stats.any_solution.classes_checked += checked[i];
    search.search_stats.any_solution.bound_rejects += bound_rejects[i];
    search.search_stats.any_solution.exact_bound_accepts += bound_accepts[i];
    search.search_stats.any_solution.exact_validations += exact_validations[i];
    search.search_stats.any_solution.memo.add(workers[i].stats.memo);
    search.search_stats.any_solution.lookahead.add(workers[i].stats.lookahead);
    search.search_stats.any_solution.nodes += workers[i].stats.nodes;
  }
  search.search_stats.run.search_seconds = std::chrono::duration<double>(
      PhaseClock::now() - validation_start).count();
  return true;
}
...
bool DfsAnagramSearch::find_completable_classes(
  ...
  if (!prepare_phase_two(
          progress_factor, allow_cache_fallback, exact_letters, true))
    return false;
  DfsAnySolutionRunner runner(*this);
  return runner.run(completable);
}
